...
관리자는 캡스톤 데모 범위에서 사용자, 강의실, AP / Wi-Fi 매핑, 네트워크 상태를 조회하고 demo presence 입력을 제어할 수 있어야 한다.
학생 / 교수 / 과목 / 강의실 / AP 매핑의 전체 생성·수정·삭제 운영 CRUD 는 후속 backlog 범위로 둔다.
...
- 강의실 목록 조회
- 강의실별 와이파이 / AP 매핑 조회
- 강의실별 threshold / AP token / demo control 처럼 현재 구현된 운영 보조 제어
- 강의실별 관측 단말 시각화
...
- 강의실과 강의 시간표 정보는 출석 판정에서 재사용 가능한 형태로 조회 가능해야 한다.
- 강의실별 와이파이 / AP 매핑 정보는 운영 화면에서 조회 가능해야 하며, 현재 MVP 의 수정 범위는 threshold / token / demo control 처럼 구현된 운영 보조 제어에 한정한다.
...
- 강의 생성 / 수정 / 삭제
- 강의실 생성 / 수정 / 삭제
- 강의실별 AP / Wi-Fi 매핑 전체 CRUD
- 권한 관리 / 감사 화면 / 운영 지표 대시보드
