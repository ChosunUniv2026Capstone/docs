...
- 교수 대시보드는 담당 강의의 과제 마감일을 월간 달력으로 표시하고, 선택 날짜의 과제명, 강의명, 마감 시간, 제출 인원 / 전체 인원을 보여줄 수 있어야 한다.
...
- 교수 출석 운영 대시보드는 담당 과목 기준의 출석 현황을 CSV 로 즉시 다운로드할 수 있어야 한다.
  - `요약본 CSV` 는 `학번`, `이름`, `출석 차시`, `결석 차시`, `지각 차시`, `공결 차시` 순서의 학생별 누계만 포함한다.
  - `전체본 CSV` 는 요약본 컬럼 뒤에 차시별 출석 상태 컬럼을 추가한다.
  - CSV 의 집계와 상태 표기는 현재 출석 화면과 동일해야 하며, `병가(sick)` 는 공결 누계에 합산한다.
  - 첫 구현에서는 CSV 외 PDF/XLSX, 기간/주차/학생 필터, 학생/관리자 다운로드, export history UI 를 추가하지 않는다.
